...
import { CATCHUP_PLAN_CONSTANTS, DifficultyLevel, PeriodType } from './constants';
...
export function getCatchupProgressMessage(analysis: CatchupAnalysis): string {
  ...
  // generateProgressMessage only handles 'weekly' | 'monthly', so handle 'daily' separately
  if (periodType === 'daily') {
    if (remainingTarget === 0) {
      return '🎉 今日の目標を達成しました！';
    }
    return `今日の目標まであと${remainingTarget}回です。`;
  }
